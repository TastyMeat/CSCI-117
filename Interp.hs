...
--sched q n p s@State{front = (tn,[]):ths} = trace ("thread completed: ") $ sched q 1 p s{front = ths} -- FIXED FROM N
...
sched q n p s@State{front = (tn,st@((m,e):st')):ths, back = back, sas = sas,
  ...
  = case exec m e s'' of
      ...
      --ESuspend -> trace ("thread suspended: " ++ show st) $ sched q 1 p s{front = ths, back = (tn,st):back}  -- Suspension
