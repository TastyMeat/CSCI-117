...
% mode(+-) ?- toBList_it([1,2],X); X = snoc(snoc(nil,1),2)
% mode(++) ?- toBList_it([1,2],snoc(snoc(nil,1),2)); X = true
% mode(-+) ?- toBList_it(X,snoc(snoc(nil,1),2)); X = [1, 2]
% mode(--) ?- toBList_it(X,Y); X = [], Y = nil
...
% mode(++) ?- fromBList_it(snoc(snoc(snoc(nil,1),2),3),[1,2,3]); true
% mode(+-) ?- fromBList_it(snoc(snoc(snoc(nil,1),2),3),X); X = [1, 2, 3]
% mode(-+) ?- fromBList_it(X,[1,2,3]); X = snoc(snoc(snoc(nil,1),2),3)
% mode(--) ?- fromBList_it(X,Y); X = nil, Y = []

sum_nodes_it(T, Out) :- go_sum_nodes([T], 0, Out).
...
% mode (+-) ?- sum_nodes_it(node(1,node(2,empty,empty),node(3,empty,empty)),X); X = 6
% mode (++) ?- sum_nodes_it(node(1,node(2,empty,empty),node(3,empty,empty)),6); true
% mode (--) ?- sum_nodes_it(X,Y); X = [], Y = 0
...
% mode (+-) ?- num_empties_it((node(2,empty,empty)),X); X = 2
% mode (-+) ?- num_empties_it(X,2); X = node(_1688,empty,empty)
% mode (++) ?- num_empties_it((node(2,empty,empty)),2); X = true
% mode (--) ?- num_empties_it(X,Y); X = empty, Y = 1

num_nodes_it(T, Out) :- go_num_nodes_it([T], 0, Out).
go_num_nodes([], Count, Count).
go_num_nodes([empty|Ts], Count, Out) :- go_num_nodes_it(Ts, Count, Out).
go_num_nodes([node(_, T1, T2)|Ts], _, Out) :- Counter is _+1, 
                                                 go_num_nodes_it([T1,T2|Ts], Counter, Out).
% mode (+-) ?- num_nodes((node(2,empty,empty)),X); X = 1
% mode (-+) ?- num_nodes(X,1); X = node(_1688,empty,empty) 
% mode (++) ?- num_nodes((node(2,empty,empty)),1); X = true
% mode (--) ?- num_nodes(X,Y); X = empty, Y = 0

sum_nodes2_it(T, Out) :- go_sum_nodes2_it([T], 0, Out).
go_sum_nodes2([], Sum, Sum). 
go_sum_nodes2([leaf(N)|Ts], Sum, Out) :- Sum_1 is Sum + N, 
                                                go_sum_nodes2(Ts, Sum_1, Out).
go_sum_nodes2([node2(N, T1, T2)|Ts], Sum, Out) :- Sum_1 is Sum + N, 
                                                         go_sum_nodes2([T1,T2|Ts], Sum_1, Out).
% mode(+-) ?- sum_nodes2(leaf(2),X); X = 2
% mode(-+) ?- sum_nodes2(X,2); X = leaf(2)
% mode(--) ?- sum_nodes2(X,Y); X = leaf(Y)
% mode(++) ?- sum_nodes2(leaf(2),2); X = true


inorder2_it(T, Out) :- go_inorder2(T, [], Out).
go_inorder2(leaf(N), List, [N|List]).
go_inorder2(node2(N, T1, T2), List, Out) :- go_inorder2(T2, List, Order),
                                            go_inorder2(T1, [N|Order], Out).
%mode (+-) ?- inorder2_it(leaf(2),X); X = [2]
%mode (-+) ?- inorder2_it(X,[2]); X = leaf(2)
%mode (--) ?- inorder2_it(X,Y); X = leaf(_1720), Y = [_1720]
%mode (++) ?- inorder2_it(leaf(2),[2]); true           
...
go_bst2(node2(N, T1, T2), Low, High) :- Low < N, N < High,
                                        go_bst2(T1, Low, N), 
                                        go_bst2(T2, N, High).
% mode(+) ?- bst2(node2(2,leaf(1),node2(4,leaf(3),leaf(5)))); true
% mode(+) ?- bst2(node2(6,leaf(7),node2(4,leaf(3),leaf(5)))); false